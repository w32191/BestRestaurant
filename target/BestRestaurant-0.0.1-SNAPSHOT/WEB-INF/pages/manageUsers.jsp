	...
	<div class="templatemo-flex-row">
		...
		<div class="templatemo-content col-1 light-gray-bg">
			...
			<div class="templatemo-content-container">
				<c:if test="${accountBean.perssion lt 99}">
					<script>
						$(function() {
							$('#insertBtnDiv').hide();
						});
								...
								<c:forEach items="${requestScope.memberList}" var="member">
									<tr>
										...
										<c:if test="${accountBean.perssion gt 97}">
											<td><a
												href=""
												class="templatemo-del-btn deleteBtn">Delete</a></td>
										</c:if>
									</tr>
								</c:forEach>
							</tbody>
				...
				<div id="dialog" title="新增帳號">
					<form action="MemberDataController" method="post" id="insertForm">
						<p>
							<label for="insertName">姓名</label> <input type="text"
								name="insertName" id="insertName" value="User Name">
						</p>
						<p>
							<label for="insertAccount">帳號</label> <input type="text"
								name="insertAccount" id="insertAccount" value="UserDefault">
						</p>
						<p>
							<label for="insertPwd">密碼</label> <input type="text"
								name="insertPwd" id="insertPwd" value="123456">
						</p>
						<p>
						...
						<p>
							<label for="insertAddress">地址</label> <input type="text"
								name="insertAddress" id="insertAddress" value="Taiwan">
						</p>
						<p>
							<label for="insertTel">電話</label> <input type="text"
								name="insertTel" id="insertTel" value="0900123456">
						</p>
						<p>
							<label for="insertPerssion">權限</label> <select
								name="insertPerssion" id="insertPerssion">
				...
				<div id="dialogUpdate" title="更新資料">
					<form action="MemberDataController" method="post" id="updateForm">
						<p>
							<label for="updateName">姓名</label> <input type="text"
								id="updateName" name="updateName">
						</p>
						<p>
							<label for="updateAccount">帳號</label> <input type="text"
								id="updateAccount" name="updateAccount">
						</p>
						<p>
							<label for="updateGender">性別</label> <select name="updateGender"
								id="updateGender">
								<option value="1" selected>男</option>
								<option value="2">女</option>
							</select>
						<p>
							<label for="updateAddress">地址</label> <input type="text"
								id="updateAddress" name="updateAddress">
						</p>
						<p>
							<label for="updateTel">電話</label> <input type="text"
								id="updateTel" name="updateTel">
						</p>
						<p>
							<label for="updatePerssion">權限</label> <select
								name="updatePerssion" id="updatePerssion">
								<c:if test="${sessionScope.memberBean.perssion gt 98}">
									<option value="99" selected>管理者</option>
								</c:if>
								<option value="1">一般帳號</option>
								<c:if test="${sessionScope.memberBean.perssion gt 1}">
									<option value="2">門市人員</option>
								</c:if>
							</select>
						</p>
